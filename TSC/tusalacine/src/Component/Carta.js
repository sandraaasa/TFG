...
const CartaPeli = ({ id, peliData }) => {
    ...
    const cambiarHeart = () => {
        if (isHeart == false) {
            ...
            axios.post(url + 'addvista', peliVista)
                .then(res => {
                    setIsHeart(!isHeart);
                })
                .catch(Error => {
    ...
    return (
        <Card title={peliData?.titulo} subTitle={subTitle} header={header} className='border-round max-w-full md:max-w-14rem xl:max-w-22rem m-2 bg-orange-50 text-gray-900'>
            <div className='grid '>

                <Inplace closable className='col-5'>
                    <InplaceDisplay><Button label="Categoría" raised icon="pi pi-info-circle border-round" /></InplaceDisplay>
                    <InplaceContent>
                ...
                <Button className='col-3 col-offset-4' icon={`pi ${!isHeart ? 'pi-eye' : 'pi-eye-slash'} `} raised rounded aria-label="Favorite" onClick={cambiarHeart} />
                <Inplace closable className='col-12'>
                    <InplaceDisplay><Button label="Sinopsis" raised icon="pi pi-info-circle border-round" /></InplaceDisplay>
                    <InplaceContent>
